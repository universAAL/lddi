...
public class SimpleType extends ManagedIndividual {
	...
	// class uri
	public static final String MY_URI = CGwDataConfigOntology.NAMESPACE + "SimpleType";
	...
	public static SimpleType valueOf(String name) {
		...
		if (name.startsWith(CGwDataConfigOntology.NAMESPACE))
			name = name.substring(CGwDataConfigOntology.NAMESPACE.length());
	...
	private SimpleType(int order) {
		super(CGwDataConfigOntology.NAMESPACE + names[order]);
		this.order = order;
	}
