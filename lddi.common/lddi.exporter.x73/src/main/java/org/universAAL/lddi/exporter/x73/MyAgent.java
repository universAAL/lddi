	...
	public void MeasurementData(String dev, String data) {
		...
		if(xmlData != null) {
			...
			try {
				...
				// two different ways to find the intended nodes. type with the first entry - value with fixed name.
				
				if(value_type.compareTo("Simple-Nu-Observed-Value")==0 || value_type.compareTo("Basic-Nu-Observed-Value")==0) {
					// Measured Value
					expr = xpath.compile("//entry/simple[name='Simple-Nu-Observed-Value' or name='Basic-Nu-Observed-Value']/value/text()");
				...
				// Unit
				expr = xpath.compile("//entry[simple/name='Simple-Nu-Observed-Value' or simple/name='Basic-Nu-Observed-Value']/meta-data/meta[@name='unit']/text()");
				nodes = (NodeList) expr.evaluate(xmlData, XPathConstants.NODESET);
				...
				// Timestamp
				expr = xpath.compile("//entries/entry/compound[name='Absolute-Time-Stamp']/entries/entry/simple[name='century']/value/text()");
				nodes = (NodeList) expr.evaluate(xmlData, XPathConstants.NODESET);
	...
	public void DeviceAttributes(String dev, String data){
		...
		if(xmlData != null) {
			...
			try {
				...
				// systemTypeSpecList
				expr = xpath.compile("//entries/entry/compound[name='System-Type-Spec-List']/entries/entry/compound/entries/entry/simple[name='type']/value/text()");
				nodes = (NodeList) expr.evaluate(xmlData, XPathConstants.NODESET);
