...
public final class ASN1PreparedElementData implements IASN1PreparedElementData {
	...
	private void setupMetadata(AnnotatedElement annotated, Class<?> objectClass) {
		if (annotated.isAnnotationPresent(ASN1SequenceOf.class)) {
			typeMeta = new ASN1SequenceOfMetadata(annotated.getAnnotation(ASN1SequenceOf.class),
					((Field) annotated).getGenericType(), annotated);
		...
		} else if (annotated.isAnnotationPresent(ASN1ObjectIdentifier.class)
				|| objectClass.equals(ObjectIdentifier.class)) {
