 ...
 */
package org.universAAL.lddi.lib.ieeex73std.org.bn.coders.ber;
...
public class BERDecoder extends Decoder {
	...
	public DecodedObject decodeSequence(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		if (!CoderUtils.isSequenceSet(elementInfo)) {
			if (checkTagForObject(decodedTag, TagClass.Universal, ElementType.Constructed, UniversalTag.Sequence,
					elementInfo)) {
			} else
				return null;
		} else {
			if (checkTagForObject(decodedTag, TagClass.Universal, ElementType.Constructed, UniversalTag.Set,
					elementInfo)) {
				isSet = true;
			} else
				return null;
		...
		if (result.getSize() != len.getValue())
			throw new IllegalArgumentException("Sequence '" + objectClass.toString() + "' size is incorrect! Must be: "
					+ len.getValue() + ". Received: " + result.getSize());
	...
	public DecodedObject decodeEnumItem(DecodedObject decodedTag, Class objectClass, Class enumClass,
		...
		if (!checkTagForObject(decodedTag, TagClass.Universal, ElementType.Primitive, UniversalTag.Enumerated,
				elementInfo))
			return null;
	...
	public DecodedObject decodeBoolean(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		if (!checkTagForObject(decodedTag, TagClass.Universal, ElementType.Primitive, UniversalTag.Boolean,
				elementInfo))
			return null;
	...
	public DecodedObject decodeInteger(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		if (!checkTagForObject(decodedTag, TagClass.Universal, ElementType.Primitive, UniversalTag.Integer,
				elementInfo))
			return null;
	...
	public DecodedObject decodeReal(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		if ((realPreamble & 0x41) == 1) {
			...
			DecodedObject<Long> mantissaEncFrm = decodeLongValue(stream,
					new DecodedObject<Integer>(szResult - szOfExp - 1));
	...
	public DecodedObject decodeChoice(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		if ((elementInfo.hasPreparedInfo() && elementInfo.hasPreparedASN1ElementInfo()
			...
			if (!checkTagForObject(decodedTag, TagClass.ContextSpecific, ElementType.Constructed,
					UniversalTag.LastUniversal, elementInfo))
				return null;
	...
	public DecodedObject decodeOctetString(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		if (!checkTagForObject(decodedTag, TagClass.Universal, ElementType.Primitive, UniversalTag.OctetString,
				elementInfo))
			return null;
		DecodedObject<Integer> len = decodeLength(stream);
	...
	public DecodedObject decodeBitString(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		if (!checkTagForObject(decodedTag, TagClass.Universal, ElementType.Primitive, UniversalTag.Bitstring,
				elementInfo))
			return null;
		DecodedObject<Integer> len = decodeLength(stream);
	...
	public DecodedObject decodeString(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		if (!checkTagForObject(decodedTag, TagClass.Universal, ElementType.Primitive,
				CoderUtils.getStringTagForElement(elementInfo), elementInfo))
			return null;
		DecodedObject<Integer> len = decodeLength(stream);
		CoderUtils.checkConstraints(len.getValue(), elementInfo);
		byte[] byteBuf = new byte[len.getValue()];
	...
	public DecodedObject decodeObjectIdentifier(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		if (!checkTagForObject(decodedTag, TagClass.Universal, ElementType.Primitive, UniversalTag.ObjectIdentifier,
				elementInfo))
			return null;
		DecodedObject<Integer> len = decodeLength(stream);
		byte[] byteBuf = new byte[len.getValue()];
