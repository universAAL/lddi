...
public abstract class Decoder implements IDecoder, IASN1TypesDecoder {
	...
	public DecodedObject decodeClassType(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		if (objectClass.isAnnotationPresent(ASN1PreparedElement.class)) {
			...
			return elementInfo.getPreparedInfo().getTypeMetadata().decode(this, decodedTag, objectClass, elementInfo,
					stream);
	...
	protected DecodedObject decodeJavaElement(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
	...
	public DecodedObject decodePreparedElement(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		IASN1PreparedElementData saveInfo = elementInfo.getPreparedInfo();

		IASN1PreparedElement preparedInstance = (IASN1PreparedElement) createInstanceForElement(objectClass,
				elementInfo);
		...
		DecodedObject result = preparedInstance.getPreparedData().getTypeMetadata().decode(this, decodedTag,
				objectClass, elementInfo, stream);
		elementInfo.setPreparedInfo(saveInfo);
		return result;
	}

	public void invokeSetterMethodForField(Field field, Object object, Object param, ElementInfo elementInfo)
			throws Exception {
	...
	public void invokeSelectMethodForField(Field field, Object object, Object param, ElementInfo elementInfo)
			throws Exception {
		if (elementInfo != null && elementInfo.hasPreparedInfo()) {
	...
	protected void initDefaultValues(Object object, ElementInfo elementInfo)
	...
	protected Object createInstanceForElement(Class objectClass, ElementInfo elementInfo) throws Exception {
		...
		if (elementInfo.hasPreparedInstance()) {
			...
			} else if (objectClass.isMemberClass() && elementInfo.getParentObject() != null
		...
		if (result == null) {
			 ...
			 * decl.setAccessible(true); result = decl.newInstance();
			 */
		}
		return result;
	}
	...
	public DecodedObject decodeSequence(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		if (maxSeqLen == -1 || maxSeqLen > 0) {
			...
			for (curFieldIdx = 0; curFieldIdx < fields.length; curFieldIdx++) {
				...
				DecodedObject<?> obj = decodeSequenceField(fieldTag, sequence, curFieldIdx, field, stream, elementInfo,
						true);
				if (obj != null) {
		...
		/*
		 * for(;curFieldIdx<fields.length; curFieldIdx++) { Field field =
		 * fields[curFieldIdx]; ElementInfo info =
		 * createSequenceFieldInfo(elementInfo, sequence, field, curFieldIdx);
	...
	protected ElementInfo createSequenceFieldInfo(ElementInfo elementInfo, Object sequenceObj, Field field,
			int fieldIdx) {
		ElementInfo info = new ElementInfo();
	...
	protected DecodedObject decodeSequenceField(DecodedObject fieldTag, Object sequenceObj, int fieldIdx, Field field,
			InputStream stream, ElementInfo elementInfo, boolean optionalCheck) throws Exception {
	...
	public DecodedObject decodeEnum(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
	...
	public DecodedObject decodeElement(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		elementInfo.setAnnotatedClass(objectClass);
		return decodeClassType(decodedTag, objectClass, elementInfo, stream);
	}

	public DecodedObject decodeBoxedType(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
