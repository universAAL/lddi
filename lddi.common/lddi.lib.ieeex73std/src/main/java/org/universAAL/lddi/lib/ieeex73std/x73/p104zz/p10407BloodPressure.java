...
public class p10407BloodPressure extends DeviceSpecialization {
	...
	private void generateBloodPressureAttributes() throws Exception {
		...
		//
		// Attribute a =
		// bloodpressureattributes.get(NomenclatureCodes.MDC_ATTR_ID_HANDLE);
		...
		entry1.setAttribute_len(new INT_U16(10)); // 3*2 SFloat + 2 bytes
													// indicating how many items
													// and 2 more with its total
													// length
	...
	private void generatePulserateAttributes() throws Exception {
		...
		BitString bs = new BitString();
		byte[] bs_byte = new byte[2];
		int mss_val = StatusCodes.MSS_AVAIL_INTERMITTENT | StatusCodes.MSS_AVAIL_STORED_DATA
				| StatusCodes.MSS_UPD_APERIODIC | StatusCodes.MSS_MSMT_APERIODIC | StatusCodes.MSS_ACC_AGENT_INITIATED;
		...
		// in page 14 of the 11073-10407 tells that there is another Mandatory
		// attribute, but in the examples it doesn't show up.
