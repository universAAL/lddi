...
public class BERCoderUtils {
	...
	public static DecodedObject<Integer> getTagValueForElement(ElementInfo info, int tagClass, int elemenType,
			int universalTag) {
		...
		if (info.hasPreparedInfo()) {
			...
			if (meta != null && meta.hasTag()) {
				result = getTagValue(tagClass, elemenType, universalTag, meta.getTag(), meta.getTagClass());
			}
		} else {
	...
	public static DecodedObject<Integer> getTagValue(int tagClass, int elemenType, int universalTag, int userTag,
			int userTagClass) {
