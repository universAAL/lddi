...
import org.universAAL.lddi.lib.ieeex73std.org.bn.coders.Encoder;
...
import org.universAAL.lddi.lib.ieeex73std.org.bn.utils.ReverseByteArrayOutputStream;
...
public class EncoderMDER<T> extends Encoder<T> {
	 ...
	 * (non-Javadoc) F.3 Byte order Refer to Figure F.1, which shows how various
	 ...
	 * MDER, e.g., CMIP* and ROSE* PDUs. The suffixed asterisk (*) indicates
	 * that MDER is used as an optimization of the ISO protocol that is based
	 * typically in binary encoding rules (BER). Multibyte structures are mapped
	 * between network and computer memory and ordered in computer memory in two
	 * basic ways, referred to as big endian and little endian. Big-endian
	 * format is consistent with NBO, but little endian is not. For example, in
	 * the last example in Figure F.1, the structure ABCD would be ordered DCBA.
	 * In this case, if big endian is the negotiated protocol, then a
	 * little-endian machine needs to swap components of these structures both
	 * to and from memory, as appropriate. Program language macros and
	...
	// returns the size in bytes of the integer and move bits properly for
	// correct storage MSB----LSB.
	...
	public int encodeOctetString(Object object, OutputStream stream, ElementInfo elementInfo) throws Exception {
		...
		if (constraint == null) { // is not a fixed octet string, we have to add
									// the length field (INT_U16) to the result
	...
	public int encodeSequence(Object object, OutputStream stream, ElementInfo elementInfo) throws Exception {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			// totalSize+= encodeSequenceField(object, i,
			// field,steam,elementInfo);
	...
	/**
	 * Group of elements of the same type. Contains: - A header with two fields:
	 * count of number of elements / length in octets of the encoded elements
	 * (not sums the header fields) - Encoded elements, one after another
	 */
	...
	@Override
	/**
	 * Compound of a INT_U16 tag (can be implicit or explicit, so it couldn't be
