...
public class BERObjectIdentifier {
	public static byte[] Encode(int[] oidArcArray) {
		...
		byte[] result = new byte[(arcLength * 5)]; // 32-bit encoding cannot
													// exceed 5 bytes each
		int nextAvailable = 0;
	...
	/// <summary>
	/// Adds encoding to passed result array. Note that result array must
	/// already have adequate capacity.
	/// </summary>
	...
	private static int EncodeOneArc(int arc, byte[] result, int nextAvailable) {
		...
		// all bytes have high-order bit one except last byte has high-order bit
		// zero
	...
	// returns an single integer arc from an list of BER encoded bytes. Note
	// that the byte array must
	...
	private static int DecodeOneArc(ArrayList berByteList) {
		...
		for (int i = 0; i <= startByteIndex; i++) {
			berBytes[i] = (byte) (berBytes[i] & 0x7f); // make continuation bit
														// zero so it doesn't
														// affect the result
			if (i != 0)
				all = all << 7;
