...
package org.universAAL.lddi.lib.ieeex73std.org.bn.coders.per;
...
import org.universAAL.lddi.lib.ieeex73std.org.bn.metadata.*;
import org.universAAL.lddi.lib.ieeex73std.org.bn.metadata.constraints.*;
import org.universAAL.lddi.lib.ieeex73std.org.bn.types.*;
import org.universAAL.lddi.lib.ieeex73std.org.bn.utils.*;
...
public class PERAlignedDecoder extends Decoder {
	...
	protected int decodeConstraintLengthDeterminant(int min, int max, BitArrayInputStream stream) throws Exception {
		if (max <= 0xFFFF) {
			...
			return (int) decodeConstraintNumber(min, max, stream); // encoding
																	// as
																	// constraint
																	// integer
		} else
	...
	protected long decodeConstraintNumber(long min, long max, BitArrayInputStream stream) throws Exception {
		...
		if (valueRange > 0 && valueRange < 256) {
			/*
			 * 1. Where the range is less than or equal to 255, the value
			 * encodes into a bit-field of the minimum size for the range. 2.
			 * Where the range is exactly 256, the value encodes into a single
			 * octet octet-aligned bit-field.
			 */
			skipAlignedBits(stream);
			...
			/*
			 * 3. Where the range is 257 to 64K, the value encodes into a two
			 * octet octet-aligned bit-field.
			 */
			skipAlignedBits(stream);
			...
			/*
			 * 4. Where the range is greater than 64K, the range is ignored and
			 * the value encodes into an octet-aligned bit-field which is the
			 * minimum number of octets for the value. In this latter case,
			 * later procedures (see 10.9) also encode a length field (usually a
			 * single octet) to indicate the length of the encoding. For the
			 * other cases, the length of the encoding is independent of the
			 * value being encoded, and is not explicitly encoded.
			 */
	...
	protected int decodeNormallySmallNumber(BitArrayInputStream stream) throws Exception {
		...
		if (bitIndicator == 0) {
			/*
			 * 10.6.1 If the non-negative whole number, "n", is less than or
			 * equal to 63, then a single-bit bit-field shall be appended to the
			 * field-list with the bit set to 0, and "n" shall be encoded as a
			 * non-negative-binary-integer into a 6-bit bit-field.
			 */
			result = stream.readBits(6);
		} else {
			/*
			 * If "n" is greater than or equal to 64, a single-bit bit-field
			 * with the bit set to 1 shall be appended to the field-list. The
			 * value "n" shall then be encoded as a semi-constrained whole
			 * number with "lb" equal to 0 and the procedures of 10.9 shall be
			 * invoked to add it to the field-list preceded by a length
			 * determinant.
			 */
			...
			if (elementInfo.getPreparedInfo().hasConstraint()) {
				...
				if (constraint instanceof ASN1ValueRangeConstraintMetadata) {
					result = decodeConstraintLengthDeterminant(
							...
							(int) ((ASN1ValueRangeConstraintMetadata) constraint).getMax(), bitStream);
	...
	public DecodedObject decodeSequence(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		/*
		 * } else { return decodeSet(decodedTag, objectClass, elementInfo,
		 * stream); }
		 */
	}
	...
	public DecodedObject decodeEnumItem(DecodedObject decodedTag, Class objectClass, Class enumClass,
		...
		// ASN1EnumItem enumObj =
		// elementInfo.getAnnotatedClass().getAnnotation(ASN1EnumItem.class);
	...
	public DecodedObject decodeInteger(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		if (objectClass.equals(Integer.class)) {
			...
			long value = 0;
			if (hasConstraint) {
				value = decodeConstraintNumber(min, max, bitStream);
			} else
	...
	public DecodedObject decodeObjectIdentifier(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		String dottedDecimal = org.universAAL.lddi.lib.ieeex73std.org.bn.coders.ber.BERObjectIdentifier.Decode(byteBuf);
