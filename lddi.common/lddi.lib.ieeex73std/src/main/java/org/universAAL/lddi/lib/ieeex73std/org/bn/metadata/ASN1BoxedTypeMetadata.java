...
import org.universAAL.lddi.lib.ieeex73std.org.bn.annotations.ASN1BoxedType;
import org.universAAL.lddi.lib.ieeex73std.org.bn.coders.ASN1PreparedElementData;
...
import org.universAAL.lddi.lib.ieeex73std.org.bn.coders.IASN1PreparedElement;
...
public class ASN1BoxedTypeMetadata extends ASN1FieldMetadata {
	...
	public int encode(IASN1TypesEncoder encoder, Object object, OutputStream stream, ElementInfo elementInfo)
		...
		if (saveInfo != null) {
			if (!saveInfo.hasTag() && elementInfo.hasPreparedASN1ElementInfo()
					&& elementInfo.getPreparedASN1ElementInfo().hasTag()) {
				ASN1ElementMetadata elData = new ASN1ElementMetadata(saveInfo.getName(), saveInfo.isOptional(),
						elementInfo.getPreparedASN1ElementInfo().hasTag(),
						elementInfo.getPreparedASN1ElementInfo().isImplicitTag(),
						elementInfo.getPreparedASN1ElementInfo().getTagClass(),
						elementInfo.getPreparedASN1ElementInfo().getTag(), saveInfo.hasDefaultValue());
	...
	public DecodedObject decode(IASN1TypesDecoder decoder, DecodedObject decodedTag, Class objectClass,
		...
		// return
		// decoder.decodeBoxedType(decodedTag,objectClass,elementInfo,stream);
		...
		if (saveElemInfo != null) {
			if (!saveElemInfo.hasTag() && elementInfo.hasPreparedASN1ElementInfo()
					&& elementInfo.getPreparedASN1ElementInfo().hasTag()) {
				ASN1ElementMetadata elData = new ASN1ElementMetadata(saveElemInfo.getName(), saveElemInfo.isOptional(),
						elementInfo.getPreparedASN1ElementInfo().hasTag(),
						elementInfo.getPreparedASN1ElementInfo().isImplicitTag(),
						elementInfo.getPreparedASN1ElementInfo().getTagClass(),
						elementInfo.getPreparedASN1ElementInfo().getTag(), saveElemInfo.hasDefaultValue());
				elementInfo.setPreparedASN1ElementInfo(elData);
			} else
				elementInfo.setPreparedASN1ElementInfo(saveElemInfo);

		}
		DecodedObject decodedResult = valueFieldMeta.getTypeMetadata().decode(decoder, decodedTag, valueField.getType(),
				elementInfo, stream);
