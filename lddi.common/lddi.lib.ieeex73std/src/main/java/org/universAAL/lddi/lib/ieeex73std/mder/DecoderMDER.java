...
public class DecoderMDER extends Decoder {
	...
	/*
	 * The encoding of an integer value is primitive, and the octets represent
	 * the value using a twos-complement binary representation for signed
	...
	public DecodedObject decodeInteger(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {

		long max = 0;
		long min = 0;
		boolean signed = false;
		ASN1ValueRangeConstraintMetadata constraint = (ASN1ValueRangeConstraintMetadata) elementInfo.getPreparedInfo()
				.getConstraint();
	...
	protected long decodeIntegerValueAsBytes(int intLen, InputStream stream, boolean signed) throws Exception {
		/*
		 * long result = 0; for (int i = intLen-1 ; i >= 0; i--) { result|=
		 * (stream.read()) << 8*i; } return result;
		 */
	...
	/*
	 * The encoding of a bit string value is primitive, and the contents octets
	 * simply represent the bits set in the bit string. Bit string lengths are
	 * constrained to 8-, 16-, or 32-bit lengths. Bit 0 in the encoding is
	 * represented by the most significant bit (MSB), bit 1 is represented by
	 ...
	 * java.io.InputStream)
	 */
	public DecodedObject decodeBitString(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		...
		ASN1SizeConstraintMetadata constraint = (ASN1SizeConstraintMetadata) elementInfo.getPreparedInfo()
				.getConstraint();
	...
	public DecodedObject decodeOctetString(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public DecodedObject decodeSequence(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		...
		if (fields != null) {
			for (int i = 0; i < fields.length; i++) {
				...
				DecodedObject objectInSequence = decodeSequenceField(null, sequence, i, field, stream, elementInfo,
						true);
	...
	/*
	 * SEQUENCE OF is encoded by a header of a count field to specify the number
	 * of encoded elements, n, that follow and a length field to specify the
	 * total number of octets, m, that follow. The length, m, shall be equal to
	 ...
	 */
	@SuppressWarnings("unchecked")
	public DecodedObject decodeSequenceOf(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
	 ...
	 * The rules for tag values are defined as follows: - Tags are implicit or
	 * explicit. - The abstract syntax for implicit tags does not include an
	 * explicit choice number and, therefore, requires a rule for assigning
	 * choice_id field values. For implicit tags, choice_id field values shall
	 * start with the value 1 and are sequential in order of the abstract syntax
	 * choices. In the example above, the choice_id field values for
	 * one_type_chosen and two_type_chosen fields are 1 and 2, respectively. -
	 * The abstract syntax for explicit tags includes an explicit choice number,
	...
	public DecodedObject decodeChoice(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
	...
	/*
	 * The ANY DEFINED BY type (ASN.1 1988/90) or the instance-of type (ASN.1
	 * 1994) is encoded by a header of a length field to specify the number of
	 * octets in the encoding of the selected value that follows. See Figure
	 ...
	 * meaningless and provides the address of the field only. Note that length
	...
	public DecodedObject decodeAny(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
		...
		if (lengthofAny > 0) {
			...
			// return the byte array with the data and its length (2 from length
			// field and the size of data)
	...
	public DecodedObject decodeObjectIdentifier(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
	...
	public DecodedObject decodeBoolean(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		throw new Exception("BOOLEAN not included in MDER");
	}

	public DecodedObject decodeReal(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		throw new Exception("REAL not included in MDER");
	}

	public DecodedObject decodeNull(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo,
			InputStream stream) throws Exception {
		throw new Exception("NULL not included in MDER");
	}

	protected DecodedObject decodeSet(DecodedObject decodedTag, Class objectClass, ElementInfo elementInfo, Integer len,
			InputStream stream) throws Exception {
