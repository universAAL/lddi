...
public class APDUProcessor {
	...
	public ApduType processAPDU(ApduType apdu) throws Exception {
		...
		// Reception of an Asociation Response (won't get any of this. We are
		// the manager and we are the only ones sending them)
		else if (apdu.isAareSelected()) {
			statemachine.transitiontoUnassociated();
			throw new Exception("Association Response received... and we shouldn't be receiving them."); // just
																											// in
																											// case
		}
		...
		// Reception of an Abort Resquest
			...
			throw new Exception("This received APDU is not from IEEE 11073"); // just
																				// in
																				// case
		}
	 ...
	 * 2- If the Agent can use the IEEE 11073-20601 protocol it is a candidate
	 * to associate. Otherwise, the association is rejected and the Manager
	 * transitions to a Unassociated state. 3- Check if the
	 * PhdAssociationInformation given by the Agent is compatible with the
	 * Manager. Otherwise, the association is rejected. 3.1- Protocol Version
	 * (1) 3.2- Encoding Rules (MDER capable) 3.3- Nomenclature Version (1) 3.4-
	 * Functional Units (normal or with test capabilities, which won't be used)
	 * 4- The Agent has to use a standard configuration. In this case, Weighing
	 * scale and Blood Pressure Monitor are supported. Otherwise, the
	 * association is rejected and the Manager transitions to a Unassociated
	 * state. 5- If the Agent is trying to establish its first connection, it
	 * will be cached. In any case, since we are using standard configurations,
	 * we will be accepting connections with KNOWN_CONFIG in the Association
	...
	private ApduType processAarq(AarqApdu aarq) throws Exception {
		...
		if (!checkStateforAarq()) {
			Logging.logAARQProcessingError(
					"AARQ received in bad time. The status is: " + statemachine.getStringChannelState());
			...
			while (i.hasNext()) {
				...
				// we look the IEEE 11073-20601 protocol. Otherwise, we reject
				// association.
				if (dp.getData_proto_id().getValue().getValue() == StatusCodes.DATA_PROTO_ID_20601) {
					...
					try {
						...
						if (isPhdValid(phd)) {
							...
							// get the configuration id in order to know if it
							// is a recognizable standard device, or uses an
							...
							if (cfg_id >= 400 && cfg_id <= 499) { // it's a
																	// standard
																	// pulsioximeter
							...
							} else if (cfg_id >= 700 && cfg_id <= 799) { // it's
																			// a
																			// standard
																			// blood
																			// pressure
								...
								statemachine.transitiontoOperating();
								break;
							} else if (cfg_id >= 800 && cfg_id <= 899) { // it's
																			// a
																			// Thermometer
							...
							} else if (cfg_id >= 1500 && cfg_id <= 1599) { // it's
																			// a
																			// weighing
																			// scale
							...
							} else if (cfg_id >= 1700 && cfg_id <= 1799) { // it's
																			// a
																			// Glucose
																			// meter
								...
								/*
								 * The following devices does not have any
								 * standard configurations. Always use extended:
								 * - Cardiovascular fitness and Activity Monitor
								 * - Strengh fitness equipment - Activity Hub
								 */

							} else {
								// the agent is not standard, but uses IEEE
								// 11073-20601. Demand its configuration.
			...
			// the agent doesn't support IEEE 11073-20601, so a communication
	...
	private boolean checkStateforAarq() throws Exception {
		...
		} else if (statemachine.getChannelState() != StateMachine20601.CHANNELSTATE_UNASSOCIATED) {
			return false;
		} else {
			return true;
		}
	...
	private boolean isPhdValid(PhdAssociationInformation phd) {
		...
		// Check if the agent uses the same Protocol Version, Encoding Rules,
		...
		if ((!Arrays.equals(protocol_version, Manager.protocol_version))
				&& (!Arrays.equals(protocol_version, fora_prot_version))) {
			Logging.logAARQProcessingError("Processing AARQ: Protocol Version not valid");
			return false;
		}

		// we have to check if the most significant bit of the first byte is 1
		// (Agent supports MDER only (0x80) or MDER and others (ex: 0xA0)
		...
		if (!(Arrays.equals(functional_units, Manager.functional_units)
				|| Arrays.equals(functional_units, test_assoc))) {
		...
		// Data Request modes
		// TODO check data request modes? in this version we will be only
		// managing agent initiated measurements
	...
	private ApduType processRlre(RlreApdu rlre) throws Exception {
		...
		if (!checkStateforRlre()) {
			...
			return null; // the Manager is in Unassociated state. Ignore the
							// Rlre
		}
	}
	...
	private ApduType processAbrt(AbrtApdu abrt) {
		Logging.logAbrtProcessing(
				"Abrt APDU received. Nothing is sent back to the Agent. Transitioning to Unassociated...");
		statemachine.transitiontoUnassociated();
		return null;
	}
	...
	private ApduType processPrst(PrstApdu prst) throws Exception {
		...
		if (!checkStateforPrst()) {
			Logging.logPrstProcessingError(
					"Data APDU received while not operating. Sending abort and transitioning to Unassociated...");
			statemachine.transitiontoUnassociated();
	...
	private boolean checkStateforPrst() throws Exception {
		if (!statemachine.getTransportState()) {
			...
			case StateMachine20601.CHANNELSTATE_ASSOCIATED_CONFIGURING_WAITINGFORCONFIG:
			case StateMachine20601.CHANNELSTATE_ASSOCIATED_CONFIGURING_CHECKINGCONFIG:
			case StateMachine20601.CHANNELSTATE_DISASSOCIATING: // this will be
																// an special
																// case. See
																// further
																// methods
	...
	private ApduType getResponseToDataApdu(DataApdu dataapdu) throws Exception {
		...
		if (choice.isRoerSelected()) {
			if (!checkRoerState()) {
				...
				// WAITING / CHECKING CONFIG : Manager may have sent a
				// roiv-cmip-get(handle=0). ee 6.3.2.6.1
				// OPERATING: Normal processing of messages. This is the normal
				// operating state.
				// No State transition in this case.
				// TODO reception of Rorj while Associated. No response will be
				// sent (we are receiving a response to a query the manager sent
				// previously)
				return null;
			}
		}
			...
			if (!checkRoivState()) {
				...
				if (statemachine
					...
					processConfiguration_Roiv_cmip_confirmed_event_report(dataapdu);
						...
						.getChannelState() == StateMachine20601.CHANNELSTATE_ASSOCIATED_CONFIGURING_CHECKINGCONFIG) {
					// The agent is sending measurements before a configuration
					// is agreed to.
					// if not a config report, send roer (no-such
					// object-instance)
					...
					if (event_type == NomenclatureCodes.MDC_NOTI_CONFIG) {
						...
						apdu = msg_generator.RoerGenerator(dataapdu.getInvoke_id(),
								StatusCodes.NO_SUCH_OBJECT_INSTANCE);
						return apdu;
					}

				} else if (statemachine.getChannelState() == StateMachine20601.CHANNELSTATE_ASSOCIATED_OPERATING) {
					// Normal processing of messages. This is the normal
					// operating state
					// TODO: normal processing of messages: only is compatible
		...
		if (choice.isRoiv_cmip_setSelected()) {
			...
			// in disassociating state: The agent sent an invoke message as the
			// manager sent an rlrq.
			// The manager has transitioned out of the Operating state and
			// therefore will not provide any response.
			return null;
		}
		...
		if (choice.isRors_cmip_confirmed_actionSelected()) {
			if (!checkRorsState()) {
				...
				// operating state.
				// No State transition in this case.
				// TODO reception of Rors while Associated. No response will be
				// sent (we are receiving a response to a query the manager sent
				// previously)
				return null;
			}
	...
	// check if Remote Remote Operation invocation Response APDU is received in
	// a proper moment
	...
	private ApduType processMeasure_Roiv_cmip_confirmed_event_reportSelected(DataApdu dataapdu) {
		...
		// Get the Handle to know what is being transmitted. 0 represents the
		// MDS. Other values, represent a Scanner or PM-Store
		...
		if (handle == 0) {
			...
			// This event provides dynamic data (typically measurements) from
			// the agent for some or all of the metric
			// objects or the MDS object that the agent supports. Data are
			// reported in the fixed format defined by the
			// Attribute-Value-Map attribute for reported metric objects or the
			// MDS object (see 7.4.5 for details on
			// event report formats). The event is triggered by an
			// MDS-Data-Request from the manager system (i.e., a
			// manager-initiated measurement data transmission), or it is sent
			// as an unsolicited message by the agent
			// (i.e., an agent-initiated measurement data transmission). For
			// agents that support manager-initiated
			// measurement data transmission, refer to 8.9.3.3.3 for information
			// on controlling the activation and/or
			// period of the data transmission. For agents that do not support
			// manager-initiated measurement data
			// transmission, refer to 8.9.3.3.2 for information on the limited
			...
			case NomenclatureCodes.MDC_NOTI_SCAN_REPORT_FIXED:
				...
				// This event provides dynamic data (typically measurements)
				// from the agent for some or all of the
				// objects that the agent supports. Data for reported objects
				// are reported using a generic attribute list
				// variable format (see 7.4.5 for details on event report
				// formats). The event is triggered by an MDS-Data-
				// Request from the manager system, or it is sent as an
				// unsolicited message by the agent. For agents that
				// support manager-initiated measurement data transmission,
				// refer to 8.9.3.3.3 for information on
				// controlling the activation and/or period of the data
				// transmission. For agents that do not support
			...
			case NomenclatureCodes.MDC_NOTI_SCAN_REPORT_VAR:
				...
				// This is the same as MDS-Dynamic-Data-Update-Fixed, but allows
				// inclusion of data from multiple
				// persons.
			case NomenclatureCodes.MDC_NOTI_SCAN_REPORT_MP_FIXED:
				...
				// This is the same as MDS-Dynamic-Data-Update-Var, but allows
				// inclusion of data from multiple
				// persons.
			...
			/*
			 * The next methods are for Scanner objets: For a simple example, in
			 * pulsioximeters. See IEEE 11073-10404, section H.6
			 */

			// Unbuf-Scan-Report-Fixed:
			// This event style is used whenever data values change and the
			// fixed message format of each object is
			...
			// This event style reports summary data about any objects and
			// attributes that the scanner monitors. The
			// event is triggered whenever data values change and the variable
			// message format (type/length/value) is
			// used when reporting data that changed.
			case NomenclatureCodes.MDC_NOTI_UNBUF_SCAN_REPORT_VAR:
				// TODO unbuffered variable report when a Scanner object is
				// used.
				break;

			// Unbuf-Scan-Report-Grouped:
			// This style is used when the scanner object is used to send the
			// data in its most compact format. The
			// Handle-Attr-Val-Map attribute describes the objects and
			// attributes that are included and the format of
			// the message.
			...
			// This is the same as Unbuf-Scan-Report-Fixed, but allows inclusion
			// of data from multiple persons.
			case NomenclatureCodes.MDC_NOTI_UNBUF_SCAN_REPORT_MP_FIXED:
				// TODO unbuffered multi-person fixed report when a Scanner
				// object is used.
				break;

			// Unbuf-Scan-Report-MP-Var:
			// This is the same as Unbuf-Scan-Report-Var, but allows inclusion
			// of data from multiple persons.
			case NomenclatureCodes.MDC_NOTI_UNBUF_SCAN_REPORT_MP_VAR:
				// TODO unbuffered multi-person variable report when a Scanner
				// object is used.
				break;

			// Unbuf-Scan-Report-MP-Grouped:
			// This is the same as Unbuf-Scan-Report-Grouped, but allows
			// inclusion of data from multiple persons.
			case NomenclatureCodes.MDC_NOTI_UNBUF_SCAN_REPORT_MP_GROUPED:
				// TODO unbuffered multi-person grouped report when a Scanner
				// object is used.
				break;

			// All of the event report styles listed in Table 18 are buffered
			// equivalents to their unbuffered counterparts in
			// 6.3.9.4.5. One difference is that the scanner buffers data over
			// the reporting interval and sends a single
			// message at the end of the interval. A second difference is that
			// the same objects and attributes are included
			// in each report regardless of whether their values have changed.
			case NomenclatureCodes.MDC_NOTI_BUF_SCAN_REPORT_FIXED:
				// TODO buffered fixed report when a Periodic Scanner object is
				// used.
				break;
			case NomenclatureCodes.MDC_NOTI_BUF_SCAN_REPORT_VAR:
				// TODO buffered variable report when a Periodic Scanner object
				// is used.
				break;
			case NomenclatureCodes.MDC_NOTI_BUF_SCAN_REPORT_GROUPED:
				// TODO buffered grouped report when a Periodic Scanner object
				// is used.
				break;
			case NomenclatureCodes.MDC_NOTI_BUF_SCAN_REPORT_MP_FIXED:
				// TODO buffered multi-person fixed report when a Periodic
				// Scanner object is used.
				break;
			case NomenclatureCodes.MDC_NOTI_BUF_SCAN_REPORT_MP_VAR:
				// TODO buffered multi-person variable report when a Periodic
				// Scanner object is used.
				break;
			case NomenclatureCodes.MDC_NOTI_BUF_SCAN_REPORT_MP_GROUPED:
				// TODO buffered multi-person grouped report when a Periodic
				// Scanner object is used.
				break;
			...
			case NomenclatureCodes.MDC_NOTI_SEGMENT_DATA:
				// TODO This event sends data stored in the Fixed-Segment-Data
				// of a PM-segment from the agent to the
				// manager. The event is triggered by the manager by the
				// Trig-Segment-Data-Xfer method. Once the data
				// transfer is triggered, the agent sends Segment-Data-Event
				// messages until the complete Fixed-Segment-
				// Data is transferred or the transfer is aborted by the manager
				// or agent. See Transfer PM-segment content
	...
	private ApduType processConfiguration_Roiv_cmip_confirmed_event_report(DataApdu dataapdu) throws Exception {
		...
		if (handle == 0) {
			...
			// This event is sent by the agent during the configuring state of
			// startup if the manager does not already
			// know the agent�s configuration from past associations. The event
			// provides static information about the
			...
			if (event_type == NomenclatureCodes.MDC_NOTI_CONFIG) {
				...
				try {
					...
					if (resp.getConfig_result().getValue().getValue() == StatusCodes.ACCEPTED_CONFIG) {
						...
						// something in this configuration is not supported. Ask
						// for another one.
