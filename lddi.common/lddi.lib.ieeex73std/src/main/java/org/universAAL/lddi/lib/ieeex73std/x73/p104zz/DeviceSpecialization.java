...
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.ConfigResult;
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.GetResultSimple;
...
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.ObservationScan;
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.ObservationScanFixed;
...
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.dim.DIM;
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.dim.MDS;
...
public class DeviceSpecialization extends MDS {
	...
	public ConfigReportRsp MDS_Configuration_Event(ConfigReport cfgreport) {
		...
		// obtain the configuration objects. Will be one for each handle
		// reported.
		...
		while (it.hasNext()) {
			...
			case NomenclatureCodes.MDC_MOC_VMO_METRIC_NU:
				try {
					...
					// if everything went OK, we accept the configuration
			...
			// // if everything went OK, we accept the configuration because we
			// can handle it.
			// response_code = StatusCodes.ACCEPTED_CONFIG;
			// } catch (Exception e) {
			// e.printStackTrace();
			...
			default:
				// if not, we reject it and keep waiting for another one (if the
				// Agent has more)
				response_code = StatusCodes.UNSUPPORTED_CONFIG;
			}
		}

		// prepare the Config Report Response to send the result of the analysis
		// of the configuration to the agent.
	...
	public void MDS_Dynamic_Data_Update_Var(ScanReportInfoVar scanreportinfovar) {
		...
		// create the object where the data will be stored for further
		// representation.
		...
		while (it.hasNext()) {
			...
			while (itlist.hasNext()) {
				...
				// from the report of the Agent, we have generic ASN objects. We
				// will have to know which Attribute it is.
				...
				// decode the ASN object as the type of its Attribute and store
				// it.
	...
	public void MDS_Dynamic_Data_Update_Fixed(ScanReportInfoFixed scanreportinfofixed) throws Exception {
		...
		while (it.hasNext()) {
			...
			if (dim.getNomenclatureCode() == NomenclatureCodes.MDC_MOC_VMO_METRIC_NU) {
				...
				// get the unit in which the information is stored (kg, cm, ...)
				// for this Handle
				...
				AttrValMap avm = (AttrValMap) numeric.getAttribute(NomenclatureCodes.MDC_ATTR_ATTRIBUTE_VAL_MAP)
						.getAttributeType();
				Iterator<AttrValMapEntry> itmap = avm.getValue().iterator();
				int size_of_attr_val_map = avm.getValue().size();

				Object[] objects = new Object[size_of_attr_val_map + 1]; // reserve
																			// another
																			// one
																			// for
																			// the
																			// unit.
				int i = 0;
				objects[objects.length - 1] = unit; // the last one is the unit,
													// so we simplify the array
				...
				while (itmap.hasNext()) {
					// get the Attribute identificator and its length. Length
					// will be used for controlling that the decoder does not
					// exceeds the bounds of the data byte array
					...
					// decode the attribute (and its stored data) and store the
					// information.
					Object object = decoder.decode(bais, ASNUtils.getAttributeClass(id));
					objects[i++] = object;
					totalreaded += length;
					// discard the already decoded data and prepare the array
					// for the next Attribute
					// if we eliminate the next two lines, nothing wrong should
					// happen, but just in case, we left them.
	...
	public void MDS_Data_Request() {
		// TODO This method allows the manager system to enable or disable
		// measurement data transmission from the agent (see 8.9.3.3.3 for a
		// description).
	...
	public void getTimeGapofDevice(GetResultSimple rors_cmip_get) {
		...
		while (it.hasNext()) {
			...
			if (abstime != null) {
				...
				System.out.println(device_year + " " + device_month + " " + device_day + " " + device_hour + " "
						+ device_min + " " + device_secs);
