...
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.SFLOAT_Type;
...
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.mdnf.FloatType;
import org.universAAL.lddi.lib.ieeex73std.x73.p20601.mdnf.SFloatType;
...
public class Measurement {
	...
	// we will store them in a specific order: time stamp of the measure, the
	// observed value and its unit.
	...
	private boolean isValue(Object object) {
		if (object.getClass().equals(BasicNuObsValue.class) || object.getClass().equals(FloatType.class)
				|| object.getClass().equals(SFloatType.class) || object.getClass().equals(SimpleNuObsValue.class)
				|| object.getClass().equals(FLOAT_Type.class) || object.getClass().equals(NuObsValue.class)
	...
	/*
	 * This method is used to represent a float point value with the same
	 * significant numbers as it was sent (set by the negative exponent value)
	 */
	...
	private void printDate(AbsoluteTime abstime, int[] time_gap) {
		...
		Logging.logMeasurement(
				"Date of measurement: " + hour + ":" + min + ":" + sec + " - " + day + "/" + month + "/" + year);
		...
		for (int i = 5; i > 0; i--) {
			...
			if (i == 2) {
				...
				if (modified_values[2] > days_in_measure_month) {
					// i.e: January 32nd = February 1st(due to a change in the
					// hour, due to a change in the minutes, due to a change in
					// de seconds)
