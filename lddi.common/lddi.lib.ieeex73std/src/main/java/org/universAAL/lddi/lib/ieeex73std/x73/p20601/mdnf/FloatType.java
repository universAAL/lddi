...
public class FloatType {
	...
	public FloatType(long measure) throws Exception {
		...
		int aux_exponent = (measureInteger >> 24); // will be like this: int (4
													// bytes) 00000000 00000000
													// 00000000 exponent
		int aux_mantissa = (measureInteger & 0x00FFFFFF); // AND operation for
															// extract the
															// mantissa, it will
															// be like this:
															// 00000000
		...
		// negative?
		if ((aux_mantissa & 0x00800000) != 0) { // if it has the MSb of the
												// mantissa to one (last 24 bits
												// of the Float)
