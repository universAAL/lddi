...
public class PERAlignedEncoder<T> extends Encoder<T> {
	...
	protected int encodeConstraintLengthDeterminant(int length, int min, int max, BitArrayOutputStream stream)
		...
		if (max <= 0xFFFF) {
			...
			return encodeConstraintNumber(length, min, max, stream); // encoding
																		// as
																		// constraint
																		// integer
		} else
	...
	protected int encodeLengthDeterminant(int length, BitArrayOutputStream stream) throws IOException {
		...
		if (length >= 0 && length < 0x80) { // NOTE 2. a) ("n" less than 128)
			stream.write(length); // a single octet containing "n" with bit 8
									// set to zero;
			result = 1;
	...
	protected int encodeConstraintNumber(long value, long min, long max, BitArrayOutputStream stream) throws Exception {
		...
		if (valueRange > 0 && valueRange < 256) {
			 ...
			 */
			result = encodeConstraintLengthDeterminant(CoderUtils.getIntegerLength(narrowedVal), 1,
					CoderUtils.getPositiveIntegerLength(valueRange), stream);
			doAlign(stream);
	...
	public int encodeLength(int value, ElementInfo elementInfo, OutputStream stream) throws Exception {
		...
		if (elementInfo.hasPreparedInfo()) {
			...
			ASN1ValueRangeConstraint constraint = elementInfo.getAnnotatedClass()
					.getAnnotation(ASN1ValueRangeConstraint.class);
			resultSize += encodeConstraintLengthDeterminant(value, (int) constraint.min(), (int) constraint.max(),
					bitStream);
			...
			// resultSize += encodeConstraintLengthDeterminant(value, 0,
			// (int)constraint.max(), bitStream);
		} else
			resultSize += encodeLengthDeterminant(value, bitStream);
		return resultSize;
	}

	protected int encodeSequencePreamble(Object object, Field[] fields, ElementInfo elementInfo, OutputStream stream)
			throws Exception {
		int resultBitSize = 0;
	...
	protected int encodeChoicePreamble(Object object, OutputStream stream, int elementIndex, ElementInfo elementInfo)
			throws Exception {
		return encodeConstraintNumber(elementIndex, 1, PERCoderUtils.getRealFieldsCount(object.getClass(), elementInfo),
	...
	public int encodeEnumItem(Object enumConstant, Class enumClass, OutputStream stream, ElementInfo elementInfo)
			throws Exception {
	...
	public int encodeBitString(Object object, OutputStream stream, ElementInfo elementInfo) throws Exception {
		/*
		 * NOTE � (Tutorial) Bitstrings constrained to a fixed length less than
		 * or equal to 16 bits do not cause octet alignment. Larger bitstrings
		 * are octet-aligned in the ALIGNED variant. If the length is fixed by
		 * constraints and the upper bound is less than 64K, there is no
		 * explicit length encoding, otherwise a length encoding is included
		 * which can take any of the forms specified earlier for length
		 * encodings, including fragmentation for large bit strings.
		 */
	...
	public int encodeObjectIdentifier(Object object, OutputStream stream, ElementInfo elementInfo) throws Exception {
		...
		byte[] buffer = org.universAAL.lddi.lib.ieeex73std.org.bn.coders.ber.BERObjectIdentifier.Encode(ia);
		if (buffer.length < 1)
			return 0;
