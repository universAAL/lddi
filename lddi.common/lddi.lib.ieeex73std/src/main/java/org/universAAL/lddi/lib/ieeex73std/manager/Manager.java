...
public class Manager<T>{
	...
	public static final byte[] functional_units = {(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00}; // always 0
	public static final byte[] encode_mder = {(byte)0x80, (byte)0x00}; //MDER

	public static final byte[] datareqmodeflags = {(byte)0, (byte)0}; // always 0
	
	public static final int MAX_APDU_SIZE = 64512;//MAX APDU SIZE FROM AGENT TO MANAGER = 63 KB (63*1024)
	public static final int MAX_APDU_TOAGENT_SIZE = 8192; //MAX APDU SIZE FROM MANAGER TO AGENT = 8 KB (8*1024)
...
//			byte[] apdu_bytes_resp = null;
//			System.out.println(statemachine.getStringChannelState() +" "+ ASNUtils.asHex(received_apdu_bytes[0]));
//			if( (received_apdu_bytes[0]==(byte)231) && (statemachine.getChannelState()==StateMachine20601.CHANNELSTATE_ASSOCIATED_OPERATING)){     // EL ERROR ESTA AQUI
//				
