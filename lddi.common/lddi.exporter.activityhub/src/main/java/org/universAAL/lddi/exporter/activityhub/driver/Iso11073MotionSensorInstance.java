...
public class Iso11073MotionSensorInstance extends ActivityHubDriver
	...
	public void incomingSensorEvent(int event) {
		this.logger.log(LogService.LOG_INFO,
				"Driver " + Iso11073MotionSensor.MY_DEVICE_CATEGORY + " for device " + this.device.getDeviceId()
						+ " received new event " + MotionSensorEvent.getMotionSensorEvent(event).toString());

		try {
			this.client.incomingSensorEvent(this.device.getDeviceId(), this.device.getDeviceCategory(), event);
		} catch (AssertionError ae) {
			this.logger.log(LogService.LOG_ERROR, "No suitable MotionSensorEvent found " + "for value: " + event);
			ae.printStackTrace();
		}
	}
