...
import org.universAAL.lddi.lib.activityhub.devicemodel.SwitchSensor;
import org.universAAL.lddi.lib.activityhub.devicemodel.SwitchSensorEvent;
...
public class Iso11073SwitchSensorInstance extends ActivityHubDriver
	...
	public void incomingSensorEvent(int event) {
		this.logger.log(LogService.LOG_INFO,
				"Driver " + Iso11073SwitchSensor.MY_DEVICE_CATEGORY + " for device " + this.device.getDeviceId()
						+ " received new event " + SwitchSensorEvent.getSwitchSensorEvent(event).toString());

		try {
			this.client.incomingSensorEvent(this.device.getDeviceId(), this.device.getDeviceCategory(), event);
		} catch (AssertionError ae) {
			this.logger.log(LogService.LOG_ERROR, "No suitable SwitchSensorEvent found " + "for value: " + event);
			ae.printStackTrace();
		}
	}
