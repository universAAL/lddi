...
public class AHServiceOntology extends ActivityHub {
	...
	public static final String DEVICE_URI_PREFIX = AHServiceOntology.ACTIVITYHUB_SERVER_NAMESPACE
			+ "controlledActivityHubDevice";

	// define service URIs
	static final String SERVICE_GET_CONTROLLED_ACTIVITYHUB_SENSORS = ACTIVITYHUB_SERVER_NAMESPACE
			+ "getControlledActivityHubSensors";

	static final String SERVICE_GET_ACTIVITYHUB_SENSOR_INFO = ACTIVITYHUB_SERVER_NAMESPACE + "getActivityHubSensorInfo";

	// define service parameter URIs
	static final String OUTPUT_CONTROLLED_ACTIVITYHUB_SENSORS = ACTIVITYHUB_SERVER_NAMESPACE
			+ "controlledActivityHubSensors";

	static final String INPUT_SENSOR_URI = ACTIVITYHUB_SERVER_NAMESPACE + "deviceURI";

	static final String OUTPUT_SENSOR_MEASUREMENT = ACTIVITYHUB_SERVER_NAMESPACE + "measurement";
	...
	static {
		OntologyManagement.getInstance().register(Activator.mc,
				new SimpleOntology(MY_URI, ActivityHub.MY_URI, new ResourceFactory() {
		...
		String[] ppLocation = new String[] { ActivityHub.PROP_CONTROLS, PhysicalThing.PROP_PHYSICAL_LOCATION };

		// Copied/refactored from smp.lighting.server !! don't know if this MUST
		// be done !?
		/**
		...
		addRestriction(
				(MergedRestriction) ActivityHub
						.getClassRestrictionsOnProperty(ActivityHub.MY_URI, ActivityHub.PROP_CONTROLS).copy(),
				ppControls, serverLightingRestrictions);

		// ActivityHub controls ActivityHubSensors
		addRestriction(MergedRestriction.getAllValuesRestriction(ActivityHub.PROP_CONTROLS, ActivityHubSensor.MY_URI),
				ppControls, serverLightingRestrictions);

		// obwohl PROP_CONTROLS vom Typ ActivityHubSensor sein muss
		// kommen auch die getSensorInfo requests rein -> ist aber ne Instance
		// Restriction
		// InstanceRestrictions sind ansch. vom Konzept her klar getrennt zu
		// TypeRestrictions
		...
		getControlledActivityHubSensors.addOutput(OUTPUT_CONTROLLED_ACTIVITYHUB_SENSORS, ActivityHubSensor.MY_URI, 0, 0,
				ppControls);
		...
		AHServiceOntology getActivityHubSensorInfo = new AHServiceOntology(SERVICE_GET_ACTIVITYHUB_SENSOR_INFO);
		...
		// - it must be exactly one (parameters #3 & #4) AdaptorPlugActuator
		// (parameter
		// #2) object
		...
		// Als input kommt die konkrete deviceURI - ist ev. eine
		// InstanceLevelRestriction
		...
		// next try
		// now we expect the real sensor type as input (e.g. from type
		// MotionSensor)
		// this works!!
		getActivityHubSensorInfo.addFilteringInput(INPUT_SENSOR_URI, ActivityHubSensor.MY_URI, 1, 1, ppControls);
		...
		getActivityHubSensorInfo.addOutput(OUTPUT_SENSOR_MEASUREMENT, ActivityHubSensorEvent.MY_URI, 1, 1,
		...
		getActivityHubSensorInfo.addOutput(OUTPUT_SENSOR_LOCATION, Location.MY_URI, 1, 1, ppLocation);
