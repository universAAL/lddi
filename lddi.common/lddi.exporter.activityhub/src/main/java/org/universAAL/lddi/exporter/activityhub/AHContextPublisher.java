...
public class AHContextPublisher {
	...
	// private static ActivityHubEventFactory eventFactory = new
	// ActivityHubEventFactory();
	...
	public AHContextPublisher(AHManager busServer) {
		...
		ContextProvider info = new ContextProvider(
				AHServiceOntology.ACTIVITYHUB_SERVER_NAMESPACE + "AHContextPublisher");
	 ...
	 * @param event
	 */
	public void activityHubSensorStateChanged(String deviceId, ActivityHubDeviceCategory activityHubDeviceCategory,
			int event) {

		// create instanceURI with trailing deviceId (is different from static
		// SensorConceptURI!)
		String instanceURI = constructActivityHubSensorURIfromLocalID(deviceId);

		// Use a factory for creation of ontology ISO-SENSOR; switch on
		// activityHubDeviceCategory

		// There is a factory in the ActivityHub ontology switching on
		// factoryIndex (int)
		// If we want to use this factory we need to know the mapping
		// factoryIndex for the device categories!
		...
		LogUtils.logInfo(Activator.mc, AHContextPublisher.class, "activityHubSensorStateChanged",
				new Object[] { "publishing a context event on the state of a " + "activityhub sensor!" }, null);

		// finally create an context event and publish it with the
		// ActivityHubSensor
