...
public abstract class ActivityHubDriver {
	...
	// protected Map<String,IsoDeviceCommand> isoDeviceCommands; //iso events
	// wie motion-detected??
	...
	public ActivityHubDriver(ActivityHubDriverClient client) {
		...
		// this.knxGroupAddressHexNotificationsMap=new HashMap<String,
		// Set<KnxNotification>>();
		// this.knxGroupAddressNotificationsMap=new HashMap<String,
		// Set<KnxNotification>>();
		// this.configure();
	...
	// /**This method must be implemented to modify the configuration in
	// according with the
	...
	// Set<DogElementDescription>
	// commandsSpecificParameters=this.device.getDeviceDescriptor().getDevCommandSpecificParams();
	// get the notification parameters.
	// Set<DogElementDescription>
	// notificationsSpecificParameters=this.device.getDeviceDescriptor().getDevNotificationSpecificParams();
	// add the specific command to the command list. They overwrite the default
	// commands
	...
	// String groupAddress=
	// command.getElementParams().get(KnxNetwork.GROUP_ADDRESS);
	// String hexValue=
	// command.getElementParams().get(KnxNetwork.COMMAND_VALUE);
	// if(realName!=null && groupAddress !=null){
	// if(hexValue!=null){
	// this.knxCommands.put(realName, new
	// KnxCommand(realName,groupAddress,hexValue));
	// }else {
	...
	// String
	// realName=DogDeviceCostants.NOTIFICATIONS_PREFIX+notification.getElementType()
	// ;
	// String groupAddress=
	// notification.getElementParams().get(KnxNetwork.GROUP_ADDRESS);
	// String hexValue=
	// notification.getElementParams().get(KnxNetwork.COMMAND_VALUE);
	// KnxNotification currentNotification;
	// //check the hexValue and distinguish two cases:
	// //the hex value is not null -->add the notification to the
	// knxGroupAddressHexNotificationsMap
	// //the hex value is null --> add the notification to the
	// knxGroupAddressNotificationsMap
	// if(hexValue!=null){
	// currentNotification=new KnxNotification(realName,groupAddress,hexValue);
	// ActivityHubDriver.addNotificationToMap(this.knxGroupAddressHexNotificationsMap,
	// currentNotification, currentNotification.getAddressHex());
	//
	// } else {
	// currentNotification=new KnxNotification(realName,groupAddress);
	// ActivityHubDriver.addNotificationToMap(this.knxGroupAddressNotificationsMap,
	// currentNotification, currentNotification.getGroupAddress());
	// }
	// ActivityHubDriver.addNotificationToMap(knxNotifications,
	// currentNotification, currentNotification.getName());
	//
	//
	// **************************
	...
	// /**
	// * This method remove, if present, the "0x" prefix of the hexValue
	// variable
	...
	// /**
	// * This method check if the map contains the key, retrieves or create an
	// HashSet<KnxNotification> associated to the key,
	// * add the notification to the set
	// * @param notificationMap
	// * @param notification
	// * @param key
	// */
	// public static void addNotificationToMap(HashMap<String,
	// Set<KnxNotification>> notificationMap,KnxNotification notification,String
	// key){
	...
	// //Implemented method2
	// for(KnxNotification
	// notificationAdd:this.knxNotifications.get(StateChangeNotification.notificationName)){
