 ...
 *
 */
public class CurtainControllerWrapper extends AbstractIntegerCallee {
	 ...
	 */
	public CurtainControllerWrapper(ModuleContext context, String itemName) {
		super(context,
				getServiceProfiles(Activator.NAMESPACE + itemName + "handler",
						new CurtainController(Activator.NAMESPACE + itemName)),
				Activator.NAMESPACE + itemName + "handler");

		Activator.logD("CurtainControllerWrapper", "Ready to subscribe");
		shDeviceName = itemName;

		// URI must be the same declared in the super constructor
		String deviceURI = Activator.NAMESPACE + itemName;
		ontDevice = new CurtainController(deviceURI);
		...
		MergedRestriction predicateRestriction = MergedRestriction
				.getFixedValueRestriction(ContextEvent.PROP_RDF_PREDICATE, CurtainController.PROP_HAS_VALUE);
		// TODO Object restr
	...
	public void publish(Event event) {
		...
		if (theValue != null) {
			CurtainController d = (CurtainController) ontDevice;
			d.setValue(theValue.intValue());
			cp.publish(new ContextEvent(d, CurtainController.PROP_HAS_VALUE));
