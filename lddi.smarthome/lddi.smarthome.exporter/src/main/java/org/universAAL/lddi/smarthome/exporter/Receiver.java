...
public class Receiver implements EventSubscriber {
	...
	public void receive(final Event event) {
		new Thread() {
			public void run() {
				System.out.println("EVENT [SRC " + event.getSource() + "] [TOP " + event.getTopic() + "] [TYP "
						+ event.getType() + "] [DATA " + event.getPayload() + "]");
				if (event instanceof ItemStateEvent) {
					ItemStateEvent stateEvent = (ItemStateEvent) event;
					GenericDevice dev = Activator.getSetOfDevices().get(stateEvent.getItemName());
					if (dev != null) {
