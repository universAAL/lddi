...
public class Activator implements BundleActivator {
	...
	ServiceReference[] references = bcontext
	...
	try {
	    ...
	    LogUtils.logWarn(moduleContext, getClass(), "start",
		    e.getMessage());
	}
		...
		for (Item item : all) {
			...
			if ( WaterFlowSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new WaterflowSensorWrapper(moduleContext,
					    item.getName()));
			} else if ( WindowSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new WindowSensorWrapper(moduleContext,
					    item.getName()));
			} else if ( PresenceSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new PresenceSensorWrapper(moduleContext,
					    item.getName()));
			} else {
			    setOfDevices.put(item.getName(),
				    new ContactSensorWrapper(moduleContext,
					    item.getName()));
			}
		    } else if (ITEM_TYPE_DIMMER.equals(type)) {
			if ( LightController.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new LightdimmerControllerWrapper(
					    moduleContext, item.getName()));
			} else if ( LightActuator.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new LightdimmerActuatorWrapper(
					    moduleContext, item.getName()));
			} else if ( LightSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new LightdimmerSensorWrapper(moduleContext,
					    item.getName()));
			} else if ( DimmerActuator.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new DimmerActuatorWrapper(moduleContext,
					    item.getName()));
			} else if ( DimmerSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new DimmerSensorWrapper(moduleContext,
					    item.getName()));
			} else {
			    setOfDevices.put(item.getName(),
				    new DimmerControllerWrapper(moduleContext,
					    item.getName()));
			}
		    }
		    if (ITEM_TYPE_NUMBER.equals(type)) {
			setOfDevices.put(item.getName(),
				new TemperatureSensorWrapper(moduleContext,
					item.getName()));
		    }
		    if (ITEM_TYPE_SHUTTER.equals(type)) {
			if ( CurtainActuator.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new CurtainActuatorWrapper(moduleContext,
					    item.getName()));
			} else if ( CurtainSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new CurtainSensorWrapper(moduleContext,
					    item.getName()));
			} else if ( CurtainController.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new CurtainControllerWrapper(moduleContext,
					    item.getName()));
			} else if ( BlindActuator.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new BlindActuatorWrapper(moduleContext,
					    item.getName()));
			} else if ( BlindSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new BlindSensorWrapper(moduleContext,
					    item.getName()));
			} else {
			    setOfDevices.put(item.getName(),
				    new BlindControllerWrapper(moduleContext,
					    item.getName()));
			}
		    }
			...
			if ( LightController.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new LightswitchControllerWrapper(
					    moduleContext, item.getName()));
			} else if ( LightActuator.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new LightswitchActuatorWrapper(
					    moduleContext, item.getName()));
			} else if ( LightSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new LightswitchSensorWrapper(moduleContext,
					    item.getName()));
			} else if ( SwitchActuator.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new SwitchActuatorWrapper(moduleContext,
					    item.getName()));
			} else if ( SwitchSensor.MY_URI.equals(subtype)) {
			    setOfDevices.put(item.getName(),
				    new SwitchSensorWrapper(moduleContext,
					    item.getName()));
			} else {
			    setOfDevices.put(item.getName(),
				    new SwitchControllerWrapper(
					    moduleContext, item.getName()));
			}
		    }
		}
		sub = new Receiver();
		sr = osgiContext.registerService(
			EventSubscriber.class.getName(), sub, null);
	    }
	}.start();
    }
	...
	LogUtils.logDebug(moduleContext, Activator.class, method,
		new String[] { log }, null);
    }
