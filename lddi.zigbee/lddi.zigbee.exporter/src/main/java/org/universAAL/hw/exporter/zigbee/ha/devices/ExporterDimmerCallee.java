...
public abstract class ExporterDimmerCallee extends ServiceCallee {
    ...
    protected ExporterDimmerCallee(ModuleContext context,
	...
	if (operation.startsWith(NAMESPACE + SERVICE_GET_VALUE)) {
		...
		response.addOutput(new ProcessOutput(NAMESPACE + OUT_GET_VLAUE,
			result));
		return response;
	    } else {
	...
	Service setValue = (Service) OntologyManagement.getInstance()
		.getResource(ontologyURI, namespace + SERVICE_SET_VALUE);
	profiles[1] = setValue.getProfile();
	profiles[1].addInput(input);
	profiles[1].addChangeEffect(ppath.getThePath(),
		inputValue.asVariableReference());
