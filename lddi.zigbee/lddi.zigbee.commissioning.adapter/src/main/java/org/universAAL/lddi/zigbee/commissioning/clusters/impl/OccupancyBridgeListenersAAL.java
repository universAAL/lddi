...
/**
 * Default implementation of the delegator class that handles the eventing of
 * the {@link OccupancyListener}
 *
 * @author <a href="mailto:stefano.lenzi@isti.cnr.it">Stefano "Kismet" Lenzi</a>
 * @author <a href="mailto:francesco.furfari@isti.cnr.it">Francesco Furfari</a>
 * @version $LastChangedRevision: 584 $ ($LastChangedDate: 2012-11-29 11:18:55
 *          +0100 (Thu, 29 Nov 2012) $)
 * @since 0.6.0
 *
 */
	...
	public void receivedReport(Dictionary<Attribute, Object> reports) {
		...
		synchronized (listeners) {
			for (OccupancyListener listener : listeners) {
				if (((Integer) reports.get(bridged)).intValue() != previousEvent) // notifying
																					// only
																					// changes
					if (lastReadingTime == 0 // first time
							|| ((lastReadingTime + minTimeBeforeNotifyStatusChange) >= System.currentTimeMillis()) // minimum
																													// time
																													// expired
					) {
