 ...
 * translation from high-level commands (from ?) to low-level command (sent to
 * the gateway).
 * 
...
public class KnxWriter {
	...
	public void write(String deviceAddress, String deviceStatus,
		...
		try {
			...
			byte[] telegram = KnxEncoder.encode(repeatBit, deviceAddress, deviceStatus,
					messageType);
			...
			core.getLogger().log(
					...
					"\n---------COMMAND FROM EMI TO HOUSE---------"
							+ byteArrayToHexString(telegram));
			sender.send(packet);
			sender.close();
	...
	/**
	 * Writes a KNX telegram into an UDP packet, using KnxEncoder methods to
	 * create the telegram
	 * 
	 * @param deviceAddress
	 *            : id of the device
	...
	public void read(String deviceId) {
		...
		try {
			...
			core.getLogger().log(
					LogService.LOG_DEBUG,
					"\n---------READING STATE FROM EMI TO HOUSE---------"
							+ byteArrayToHexString(telegram));
