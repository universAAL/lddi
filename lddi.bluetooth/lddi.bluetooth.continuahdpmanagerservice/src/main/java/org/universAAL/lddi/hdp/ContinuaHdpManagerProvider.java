...
public class ContinuaHdpManagerProvider extends ServiceCallee {
	...
	public ServiceResponse handleCall(ServiceCall call) {
		...
		if (call == null) {
			...
			if (operation == null) {
				return null;
			} else if (operation.startsWith(ProvidedContinuaHdpManagerService.SERVICE_SWITCH_ON)) {
				...
				manager = new hdpManager(((ContinuaHealthDevice) input).getDataType(),
						((ContinuaHealthDevice) input).getMAC(), ctx);
				manager.init();
				return new ServiceResponse(CallStatus.succeeded);
			} else if (operation.startsWith(ProvidedContinuaHdpManagerService.SERVICE_SWITCH_OFF)) {
