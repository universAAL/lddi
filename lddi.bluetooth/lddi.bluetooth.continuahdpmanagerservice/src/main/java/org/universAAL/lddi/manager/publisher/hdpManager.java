...
public class hdpManager implements hdpManagerListener {
	...
	// Dinamic library name and path where it can be found (IMPORTANT: without
	// "lib" prefix)
	// TODO Please, check the name and location of the native library (/lib
	// folder)
	...
	// Trusted property of remote device (Property options: Trusted[boolean],
	// Blocked[boolean], Alias[string])
	private static String trustedProperty = "Trusted";

	// HDP channel properties. Options:
	// - dataTypeValue = depends on the type/profile of the remote device.
	// You'll find the right value at x073 specs (weight scale = 0x100F, pulse
	// oximeter = 0x1004, blood pressure monitor = 0x1007, thermometer = 0x1008,
	// glucose meter = 0x1011, etc)
	// - roleValue = "Source" or "Sink" (which role will play our application?).
	// - shorDescriptionValue = some text (optional)
	// - channelTypeValue = "Reliable" or "Streaming" (just for "Sources",
	// optional otherwise)
	...
	// Remote device path (real sample:
	// /org/bluez/1134/hci0/dev_00_09_1F_80_0A_E0)
	private static String remoteDevicePath = null;

	// Data channel path (real sample:
	// /org/bluez/1092/hci0/dev_00_09_1F_80_0A_E0/chan65279)
	...
	/** Constructor */
	public hdpManager(String str1, String str2, BundleContext context) {
		ctx = context;
		remoteDeviceType = str1;
	...
	public void onDataReceived() {
		...
		if (fsm.getStringChannelState().equals("ASSOCIATED - OPERATING")) {
			if (remoteDeviceType.equals("BloodPressureMonitor")) {
				if ((Measurement.hrMeasurement != null) && (Measurement.sysMeasurement != null)
						&& (Measurement.diaMeasurement != null)) {
	...
	public void exit(String str1, String str2) {
		if ((str1.equals(CONTINUA_DEVICE)) && (str2.equals(macAddressRemoteDevice))) {
			if (fsm != null)
				fsm.transportDeactivate();
