...
import org.universAAL.lddi.iso11073.activityhub.devicemodel.UsageSensor;
import org.universAAL.lddi.iso11073.activityhub.devicemodel.UsageSensorEvent;
 ...
 * Tracks on the UsageSensor device service passed in the attach method 
 * in Iso11073UsageSensorDriver class.
 * This instance is passed to the consuming client (e.g. uAAL exporter bundle).
 * When the UsageSensor device service disappears, this driver is removed
...
public class Iso11073UsageSensorInstance extends ActivityHubDriver 
	 ...
	 */
	public Iso11073UsageSensorInstance(BundleContext c, 
	...
	public void incomingSensorEvent(int event) {
		this.logger.log(LogService.LOG_INFO, "Driver " + Iso11073UsageSensor.MY_DEVICE_CATEGORY +
				" for device " + this.device.getDeviceId() + " received new event " + 
				UsageSensorEvent.getUsageSensorEvent(event).toString());

		try {
			this.client.incomingSensorEvent(this.device.getDeviceId(), this.device.getDeviceCategory(), event);
		} catch (AssertionError ae) {
			this.logger.log(LogService.LOG_ERROR, "No suitable UsageSensorEvent found " +
					"for value: " +	event);
			ae.printStackTrace();
		}		
	}
