...
public class FS20Server extends ServiceCallee { 
	 ...
	 * @param context = the given ModuleContext
	 */
	...
	public ServiceResponse handleCall(ServiceCall call) {
		...
		if (operation.startsWith(ProvidedFS20Service.SERVICE_ACTIVATE_GONG)) {
			...
			Object deviceURI = call
					.getInputValue(ProvidedFS20Service.INPUT_URI_GONG);
		...
		if (operation.startsWith(ProvidedFS20Service.SERVICE_TURN_ON_FS20ST)) {
			...
			Object deviceURI = call
					.getInputValue(ProvidedFS20Service.INPUT_URI_FS20ST);
		...
		if (operation.startsWith(ProvidedFS20Service.FS20_SERVER_NAMESPACE
				+ "displayAnimation")) {

			Object deviceURI = call
					.getInputValue(ProvidedFS20Service.INPUT_URI_DISPLAY);
			...
			for (int i = 1; i < 13; i++) {
				if (operation
						.startsWith(ProvidedFS20Service.FS20_SERVER_NAMESPACE
								+ "displayAnimation"
								+ new Integer(i).toString())) {
					return startAnimation(((LightActuator) deviceURI).getURI(),
							animationmap.get(i));
				}
			}
	...
	private ServiceResponse getFS20Gong() {
		...
		sr.addOutput(new ProcessOutput(
				ProvidedFS20Service.OUTPUT_RadioControlledBell, list));
		return sr;
	...
	private ServiceResponse getFS20Display() {
		...
		sr.addOutput(new ProcessOutput(
				ProvidedFS20Service.OUTPUT_FS20DISPLAYS, list));
	...
	private ServiceResponse getFS20FMS() {
		...
		sr.addOutput(new ProcessOutput(
				ProvidedFS20Service.OUTPUT_FS20FMS, list));
	...
	private ServiceResponse getFS20ST() {
		...
		sr.addOutput(new ProcessOutput(
				ProvidedFS20Service.OUTPUT_FS20ST, list));
...
//		list.clear();
		...
		sr.addOutput(new ProcessOutput(
				ProvidedFS20Service.OUTPUT_FS20PIRX, list));
	 ...
	 * 
	 * @param uri = the URI of the FS20 device
	 ...
	 * 
	 * @param uri = the URI of the FS20ST device
	 * @return returns the CallStatus
	 */
	 ...
	 * 
	 * @param uri = the URI of the FS20SIG device
	 * @return returns the CallStatus
	 */
	...
	/**
 	* Starts the animation with a given number on a
 	* FS20RGBSA device by URI
 	* 
 	* @param animation
	* @param uri = the URI of the FS20ST device
	* @return returns the CallStatus
	*/
	...
	/**
	 * initialize animation map help method to operate the device command @FS20Display
	 */
