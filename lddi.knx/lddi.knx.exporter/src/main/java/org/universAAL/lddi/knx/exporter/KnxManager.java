...
public class KnxManager implements IKnxDriverClient {
	...
	public void incomingSensorEvent(String groupDeviceId, int datapointTypeMainNubmer, int datapointTypeSubNubmer,
		...
		for (Iterator<KnxContextPublisher> i = contextListeners.iterator(); i.hasNext();)
			((KnxContextPublisher) i.next()).publishKnxEvent(groupDeviceId, datapointTypeMainNubmer,
					datapointTypeSubNubmer, value);

		if (contextListeners.isEmpty())
			this.logger.log(LogService.LOG_WARNING,
					"No context providers available for KNX groupDevice " + groupDeviceId);
	...
	public void sendSensorEvent(String groupDeviceId, boolean value) {
		...
		if (knxdriver == null) {
			this.logger.log(LogService.LOG_WARNING,
					"No KnxDriver available for " + groupDeviceId + ". Cannot forward event " + value + " to KNX bus!");
		} else {
			...
			case 5:
				try {
					...
					this.logger.log(LogService.LOG_ERROR,
							"Datatype DPT5 (8-bit unsigned int) " + "doesn't suit for groupDevice " + groupDeviceId
									+ "! Cannot forward event " + value + " to KNX bus!");
				}
				return;
			case 9:
				try {
					...
					this.logger.log(LogService.LOG_ERROR,
							"Datatype DPT9 (2-byte float value) " + "doesn't suit for groupDevice " + groupDeviceId
									+ "! Cannot forward event " + value + " to KNX bus!");
				}
				return;
			default:
				this.logger.log(LogService.LOG_ERROR,
						"Neither datapoint type 5 (8-bit unsigned int) "
								+ "nor dpt 9 (2-byte float value) found for groupDevice " + groupDeviceId
								+ "! Cannot forward event " + value + " to KNX bus!");
				return;
			}
		}
	...
	public void addDriver(String groupDeviceId, KnxGroupDeviceCategory knxGroupDeviceCategory, KnxDriver knxDriver) {
		...
		if (oldDriver != null) {
			this.logger.log(LogService.LOG_WARNING,
					"An existing KNX driver " + "is now replaced by a new one for groupDevice " + groupDeviceId
							+ " and category: " + knxGroupDeviceCategory);
		}

		this.logger.log(LogService.LOG_INFO,
				"new KNX driver added for groupDevice " + groupDeviceId + " and category: " + knxGroupDeviceCategory);
	}

	/** {@inheritDoc} */
	public void removeDriver(String groupDeviceId, KnxDriver knxDriver) {
		this.driverList.remove(groupDeviceId);
		this.logger.log(LogService.LOG_INFO, "removed KNX driver for " + "groupDevice " + groupDeviceId);
	}
