...
public class KnxDpt1Instance extends KnxDriver implements IKnxDpt1, ServiceTrackerCustomizer, Constants {
	...
	public Object addingService(ServiceReference reference) {
		...
		if (this.setgroupDevice(knxDev))
			...
			this.logger.log(LogService.LOG_ERROR,
					"Error coupling " + IKnxDpt1.MY_DEVICE_CATEGORY + " driver to groupDevice "
							+ this.groupDevice.getGroupAddress() + ". No appropriate " + "ISO device created!");
			return null;
		}
		...
		if (this.isoDeviceCategory != null) {
			...
			// // get knx-iso mapping properties for my groupDevice according to
			// groupAddress
			// String isoDeviceType = (String)
			// this.knxIsoMappingProperties.get("isoDeviceType");
			// KnxGroupDeviceCategory isoDeviceCategory =
			...
			if (this.groupDevice.getGroupAddress() != null) {
				...
				this.logger.log(LogService.LOG_INFO,
						"KNX to ISO mapping parameter found for " + "groupDevice " + this.groupDevice.getGroupAddress()
								+ " with KNX datapoint type " + this.groupDevice.getDatapointType() + " : "
								+ isoDeviceCategory);

				// // check deviceLocation property
				// String loc = (String)
				// this.knxIsoMappingProperties.get("deviceLocation");
				// ActivityHubLocation isoDeviceLocation =
				// ActivityHubLocationUtil.
				// toActivityHubLocation(loc);
				// if (isoDeviceLocation == null) {
				// this.logger.log(LogService.LOG_WARNING, "Location for KNX
				// groupDevice " +
				...
				// +
				// "Location info from ETS:: Name: " +
				// this.groupDevice.getDeviceLocation() +
				// "; Type: " + this.groupDevice.getDeviceLocationType() +
				// "; Description: " +
				// this.groupDevice.getDeviceLocationDescription());

				// create appropriate ActivityHub groupDevice
				this.activityHubSensor = ActivityHubFactory.createInstance(isoDeviceCategory,
						// isoDeviceLocation,
						null, this.groupDevice.getGroupAddress(), this.logger);
				if (this.activityHubSensor == null) {
					this.logger.log(LogService.LOG_ERROR,
							"Error on creating ActivityHubSensor " + " for device category: " + isoDeviceCategory
									+ " with deviceId: " + this.groupDevice.getGroupAddress());
					return null;
				}
				...
				propDeviceService.put(org.osgi.service.device.Constants.DEVICE_CATEGORY, isoDeviceCategory.toString());
				// more possible properties from OSGi: description, serial, id

				// this.myIsoDeviceRegistration =
				this.context.registerService(org.osgi.service.device.Device.class.getName(), this.activityHubSensor,
						propDeviceService);

				this.logger.log(LogService.LOG_INFO, "Registered ISO device " + this.activityHubSensor.getDeviceId()
						+ " in OSGi registry under " + "device category: " + isoDeviceCategory);
	...
	public void newMessageFromKnxBus(byte[] event) {
		// try to display event byte readable. No good: Byte.toString(byte),
		// Integer.toHexString(byte)
		this.logger.log(LogService.LOG_INFO,
				"Driver " + IKnxDpt1.MY_DEVICE_CATEGORY + " for groupDevice " + this.groupDevice.getGroupAddress()
						+ " with knx datapoint type " + this.groupDevice.getDatapointType()
						+ " received new knx message " + KnxEncoder.convertToReadableHex(event));
		...
		if (this.activityHubSensor != null) {
			...
			// // map on dpt!!
			// String dptString = "_" +
			// this.device.getDatapointType().replace('.', '_');
			// this.logger.log(LogService.LOG_INFO, "Datapoint type String: " +
			// dptString);
			//
			// String constName = IKnxDpt1.DEFAULT_VALUE_ON
			//
			/**
			 * KNX datapoint type 1.*** is a 1-bit signal; therefore only on/off
			 * is forwarded to ISO devices!
			 */
			...
			this.logger.log(LogService.LOG_ERROR,
					"Driver " + IKnxDpt1.MY_DEVICE_CATEGORY + " for groupDevice " + this.groupDevice.getGroupAddress()
							+ " lost its mapping ISO11073 sensor! " + "Cannot forward incoming message!");
		}
	}

	/**
	 * @param reference
	 *            KNX groupDevice service
	 */
