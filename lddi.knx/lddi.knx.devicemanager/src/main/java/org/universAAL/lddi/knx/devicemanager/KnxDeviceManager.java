...
public class KnxDeviceManager implements ManagedService, ServiceTrackerCustomizer {
	...
	/**
	 * List of registered devices key = groupAddress value =
	...
	String filterQuery = String.format("(%s=%s)", org.osgi.framework.Constants.OBJECTCLASS,
			IKnxNetwork.class.getName());
			...
			try {
				...
				if (knxConfigFile != null && knxConfigFile != "") {
					...
					if (this.knxImportedGroupAddresses.isEmpty()) {
						this.logger.log(LogService.LOG_WARNING, "No KNX devices found in configuration!!!!");
					} else {
						this.logger.log(LogService.LOG_DEBUG,
								"Knx devices found in configuration: " + this.knxImportedGroupAddresses.toString());
					}
					...
					for (KnxGroupAddress knxGroupAddress : knxImportedGroupAddresses) {
						...
						if (checkKnxGroupAddress(knxGroupAddress)) {
							...
							ServiceRegistration knxGA = this.groupDeviceRegistrationList
									.get(knxGroupAddress.getGroupAddress());
							if (knxGA != null) {
							...
							// create appropriate groupDevice from dpt main
							// number
							KnxGroupDevice knxGroupDevice = KnxGroupDeviceFactory.getKnxGroupDevice(dptMainNumber);

							// startover with next groupDevice if no appropriate
							// KnxGroupDevice implementation found!
							if (knxGroupDevice == null) {
								this.logger.log(LogService.LOG_WARNING,
										"KNX data type " + knxGroupAddress.getDpt() + " is not supported yet!"
							...
							propDeviceService.put(org.osgi.service.device.Constants.DEVICE_CATEGORY,
							...
							ServiceRegistration deviceServiceReg = this.context.registerService(
									org.osgi.service.device.Device.class.getName(), knxGroupDevice, propDeviceService);

							this.logger.log(LogService.LOG_INFO,
									"Registered KNX groupDevice " + knxGroupDevice.getGroupDeviceId() +
									// " (" +
				...
				this.logger.log(LogService.LOG_ERROR,
						"KNX configuration xml file " + knxConfigFile + " could not be opened!");
				e.printStackTrace();
	...
	public void stop() {
		...
		// this.unregisterManagedService(); is done automatically during bundle
		// stop
		...
		// this.logger.log(LogService.LOG_WARNING, "groupDeviceList is not empty
		// after stopping DevMan! Still count "
